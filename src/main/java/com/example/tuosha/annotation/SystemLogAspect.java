package com.example.tuosha.annotation;

import com.example.tuosha.Util.MyUtil;
import com.example.tuosha.Util.WebUtils;
...
public class SystemLogAspect {
    //注入Service用于把日志保存数据库    
    @Resource
    private TbLogService logServiceImp;
    //本地异常日志记录对象    
    private static final Logger logger = LoggerFactory.getLogger(SystemLogAspect.class);

    //Controller层切点    
    @Pointcut("@annotation(com.example.tuosha.annotation.SysLog)")
    ...
    public void doBefore(JoinPoint joinPoint) {
        ...
        //读取session中的用户    
        //TbAdminEntity user = (TbAdminEntity)SecurityUtils.getSubject().getPrincipal();
        //请求的IP    
        ...
        try {
            ...
            log.setCreateTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse((MyUtil.getNowDateStr2())));
            log.setIp(ip);
